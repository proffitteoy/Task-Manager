...
function buildCoverParticleGeometry(resolution, seed) {
...
`;
const bloomVertexShader = vertexShader.replace("uniform float uMouseActive, uPixel, uColorMixT, uLoading;", "uniform float uMouseActive, uPixel, uColorMixT, uLoading, uBloomSize;").replace("gl_PointSize = sz * uPixel * uPointScale;", "gl_PointSize = sz * uPixel * uPointScale * uBloomSize;");
...
`;
function deriveSyntheticBands(state, elapsed) {
  ...
  return {
    ...
    mid: clampRange((0.1 + Math.pow(airy, 2.2) * 0.44 + transient * 0.08) * volume, 0, 0.78),
    treble: clampRange((0.08 + Math.pow(1 - beatWave, 3.2) * 0.3 + progress * 0.1) * volume, 0, 0.68),
    beat: clampRange((transient * 0.74 + syncopation) * volume, 0, 1),
    energy: clampRange((0.18 + transient * 0.22 + airy * 0.26) * volume, 0, 0.9)
  };
}
function MineradioParticleField({
  coverUrl,
  isPlaying,
  progress,
  currentTime,
  ...
  const stateRef = useRef({ isPlaying, progress, currentTime, volume });
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    camera.lookAt(0, 0, 0);
    const resolution = width < 640 || reduceMotion ? 0.78 : 1;
    ...
    let coverTexture = makeSolidTexture("#111827");
    ...
    const uniforms = {
      ...
      uSpeed: { value: reduceMotion ? 0.08 : 1 },
      uTwist: { value: 0.12 },
    ...
    const render = (now) => {
      ...
      uniforms.uSpeed.value += ((reduceMotion ? 0.06 : active ? 1 : 0.42) - uniforms.uSpeed.value) * ease;
...
export {
  MineradioParticleField as default
};
